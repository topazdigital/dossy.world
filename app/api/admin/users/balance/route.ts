...
import { updateUserBalance, findUserById, createTransaction, recordAudit } from '@/lib/db'
...
export async function POST(request: NextRequest) {
  try {
    ...
    await recordAudit({
      ...
      action: type === 'credit' ? 'user.balance_credit' : 'user.balance_debit',
      target_type: 'user',
      target_id: userId,
      details: { amount, target_username: user.username, new_balance: updatedUser.balance },
    })
