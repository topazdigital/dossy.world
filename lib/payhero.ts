...
function getCallbackUrl(): string {
  if (process.env.PAYHERO_CALLBACK_URL) return process.env.PAYHERO_CALLBACK_URL
  const base =
...
function isConfigured(): boolean {
  return Boolean(process.env.PAYHERO_BASIC_AUTH && process.env.PAYHERO_CHANNEL_ID)
}
