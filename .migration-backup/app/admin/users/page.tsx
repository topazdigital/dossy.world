...
export default function AdminUsersPage() {
  ...
  const botUsers = filteredUsers.filter(u => u.is_bot)
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Manage players and their balances</p>
        </div>
        ...
        <Card className="border-[var(--border)] bg-card/50">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              ...
              <Users className="size-8 text-[var(--neon-cyan)]" />
            </div>
          </CardContent>
        </Card>
        <Card className="border-[var(--border)] bg-card/50">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{botUsers.length}</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  KES {formatCurrency(realUsers.reduce((sum, u) => sum + u.balance, 0))}
                </p>
              </div>
              <CircleDollarSign className="size-8 text-[var(--neon-green)]" />
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-[var(--border)] bg-card/50">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Users className="size-5 text-[var(--neon-cyan)]" />
            All Users
          </CardTitle>
        </CardHeader>
        <CardContent>
                ...
                {realUsers.length === 0 ? (
                  ...
                  realUsers.map((user, index) => (
                    ...
                    >
                      ...
                      <TableCell className="font-medium">
                        KES {formatCurrency(user.balance)}
                      </TableCell>
                      <TableCell>
                        {user.is_banned ? (
      ...
      {/* User Details / Balance Adjustment Dialog */}
      <Dialog open={!!selectedUser} onOpenChange={() => setSelectedUser(null)}>
        <DialogContent className="bg-card border-[var(--border)]">
          <DialogHeader>
            <DialogTitle>User: {selectedUser?.username}</DialogTitle>
          </DialogHeader>
          {selectedUser && (
            <div className="space-y-6 pt-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium text-[var(--neon-green)]">KES {formatCurrency(selectedUser.balance)}</p>
                </div>
                <div>
                ...
                <div className="space-y-3">
                  <div className="flex gap-2">
                    <Button
                      ...
                      className={adjustType === 'credit' ? 'bg-green-600' : ''}
                    >
                    ...
                    <Button
                      ...
                      className={adjustType === 'debit' ? 'bg-red-600' : ''}
                    >
