...
const ACTION_COLORS: Record<string, string> = {
  'settings.update': 'text-[var(--neon-cyan)]',
  'user.ban': 'text-red-400',
  'user.unban': 'text-emerald-400',
  'user.balance_credit': 'text-emerald-400',
  'user.balance_debit': 'text-amber-400',
  'withdrawal.approve': 'text-emerald-400',
  'withdrawal.reject': 'text-red-400',
}
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <ScrollText className="size-6 text-[var(--neon-cyan)]" />
            Audit Log
          </h1>
      ...
      <Card className="border-[var(--border)] bg-card/50">
        <CardHeader>
          ...
          <CardDescription>Newest first. Showing up to 500 entries.</CardDescription>
        </CardHeader>
        <CardContent>
          {logs.length === 0 ? (
            <p className="text-muted-foreground text-sm">
              {isLoading ? 'Loading…' : 'No admin actions recorded yet.'}
            </p>
          ) : (
                ...
                <tbody>
                  {logs.map(log => (
                    <tr key={log.id} className="border-b border-[var(--border)]/40 align-top">
                      ...
                      <td className={`py-2 pr-4 font-mono ${ACTION_COLORS[log.action] || ''}`}>
                        {log.action}
                      </td>
