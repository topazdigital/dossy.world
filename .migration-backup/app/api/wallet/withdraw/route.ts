...
export async function POST(request: NextRequest) {
  try {
    ...
    // Withdrawals must go to the same M-Pesa phone the account is bound to.
    // Stops "wash" abuse where deposits come from one number and winnings
    // are sent to another.
    if (user.bound_phone && user.bound_phone !== normalizedPhone) {
      return NextResponse.json(
        {
          error: `Withdrawals must go to your bound phone ending in ${user.bound_phone.slice(-4)}.`,
        },
        { status: 400 },
      )
    }
