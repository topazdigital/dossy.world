...
import { ArrowLeft, TrendingUp, ArrowUpFromLine, ArrowDownToLine, History, Wallet, Gamepad2, Loader2 } from 'lucide-react'
...
export default function HistoryPage() {
  ...
  const fetchHistory = async () => {
    ...
    try {
      const [txRes, buysRes] = await Promise.all([
        fetch(`${apiBase}/api/user/transactions`),
        fetch(`${apiBase}/api/user/buys`),
      ])
      if (txRes.ok) {
  ...
  const formatDate = (dateStr: string) =>
    new Date(dateStr).toLocaleDateString('en-KE', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })
        ...
        <Tabs defaultValue="transactions" className="space-y-4">
            ...
            {isLoading ? (
              ...
              <AnimatePresence>
                {transactions.map((tx, index) => (
                  <motion.div key={tx.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                    <Card className="border-border/50 bg-card/50">
                      <CardContent className="flex items-center justify-between p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-muted-foreground text-xs">{tx.description || formatDate(tx.created_at)}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className={`font-semibold ${tx.amount > 0 ? 'text-[var(--neon-green)]' : 'text-foreground'}`}>
          ...
          <TabsContent value="buys" className="space-y-3">
            {isLoading ? (
              ...
              <AnimatePresence>
                {buys.map((buy, index) => (
                  <motion.div key={buy.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                    <Card className="border-border/50 bg-card/50">
                      <CardContent className="flex items-center justify-between p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">Round #{buy.round?.round_number || '?'}</p>
                            <p className="text-muted-foreground text-xs">Buy: {formatAmount(buy.amount)}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          {buy.is_paid && buy.payout_amount ? (
                            <>
                              <p className="font-semibold text-[var(--neon-green)]">+{formatAmount(buy.payout_amount - buy.amount)}</p>
                              <Badge className="bg-[var(--neon-green)]/20 text-[10px] text-[var(--neon-green)]">Paid</Badge>
                            </>
                          ) : (
                            <>
                              <p className="text-muted-foreground font-semibold">Pending</p>
